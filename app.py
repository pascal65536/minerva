...
from behoof import load_json, calculate_md5
...
# Настройка хранения БД в папке instance
basedir = Path(__file__).parent.resolve()
instance_dir = basedir / "instance"
os.makedirs(instance_dir, exist_ok=True)

# Формируем абсолютный путь. as_posix() нужен для корректной работы на Windows и Linux
app.config["SQLALCHEMY_DATABASE_URI"] = f"sqlite:///{(instance_dir / 'linter_cache.db').as_posix()}"
        ...
        for checker_data in checks:
            ...
            if not checker_code_obj:
                ...
                checker_code_obj = CheckerCode.create(
                    ...
                    code=code,
                    group_key=group_key,
                    group_color=checker_data.get("color", "primary"),
                    ...
                    group_more_info=checker_data.get("more_info"),
                    group_issue_severity=checker_data.get("issue_severity"),
                    group_issue_confidence=checker_data.get("issue_confidence"),
                    group_issue_cwe_id=checker_data.get("issue_cwe_id"),
                    group_issue_cwe_link=checker_data.get("issue_cwe_link"),
                )
...
def _update_group_field(group, field_name, new_value):
    current_val = getattr(group, field_name, None)
    old_str = str(current_val).strip() if current_val else ""
    new_str = str(new_value).strip() if new_value else ""

    if new_str != old_str:
        setattr(group, field_name, new_str if new_str else None)
        return True
    return False
...
def group_action():
    ...
    if action == "group" and group_keys:
        ...
        try:
            ...
            flash(f"Группы успешно объединены под ID Epoch: '{main_group.group_key}'.", "success")
        except Exception as e:
        ...
        try:
            ...
            if current_key:
                refresh_file_cache_by_key(current_key)
            flash(
                f"Группа разгруппирована на {len(new_groups)} индивидуальных ошибок с новыми UUID.",
                "success",
            )
        except Exception as e:
        ...
        new_translate = request.form.get("edit_translate", "").strip()
        new_descriptions = request.form.get("edit_descriptions", "").strip()
        new_color = request.form.get("edit_color", "").strip()
        new_source_url = request.form.get("edit_source_url", "").strip()
        new_best_practice = request.form.get("edit_best_practice", "").strip()
        
        # === НОВЫЕ ПОЛЯ ===
        new_more_info = request.form.get("edit_more_info", "").strip()
        new_issue_severity = request.form.get("edit_issue_severity", "").strip()
        new_issue_confidence = request.form.get("edit_issue_confidence", "").strip()
        new_issue_cwe_id = request.form.get("edit_issue_cwe_id", "").strip()
        new_issue_cwe_link = request.form.get("edit_issue_cwe_link", "").strip()

        changed = False
        changed |= _update_group_field(group, "name", new_name)
        changed |= _update_group_field(group, "translate", new_translate)
        changed |= _update_group_field(group, "descriptions", new_descriptions)
        changed |= _update_group_field(group, "source_url", new_source_url)
        changed |= _update_group_field(group, "best_practice", new_best_practice)
        
        # === СОХРАНЕНИЕ НОВЫХ ПОЛЕЙ ===
        changed |= _update_group_field(group, "more_info", new_more_info)
        changed |= _update_group_field(group, "issue_severity", new_issue_severity)
        changed |= _update_group_field(group, "issue_confidence", new_issue_confidence)
        changed |= _update_group_field(group, "issue_cwe_id", new_issue_cwe_id)
        changed |= _update_group_field(group, "issue_cwe_link", new_issue_cwe_link)

        valid_colors = ("primary", "secondary", "success", "danger", "warning", "info", "dark")
        if new_color in valid_colors and new_color != group.color:
